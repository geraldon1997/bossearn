        ...
        <div class="col-md-6 m-auto">
            <form action="<?php echo ADDNEWS; ?>" method="post" class="form-wrapper" enctype="multipart/form-data">
                <select name="newstypeid" class="form-control">
                    <option value="<?php if (isset($data['data']['newstyped'])) {
                        echo $data['data']['newstypeid'];
                                   } ?>">choose type of post</option>
                    <?php foreach (News::newsType() as $newstype) : ?>
                    <option value="<?php echo $newstype['id']; ?>"><?php echo $newstype['type']; ?></option>
                    <?php endforeach; ?>
                </select>

                <input type="text" name="title" id="" class="form-control" placeholder="title of post" value="<?php if (isset($data['data']['title'])) {
                    echo $data['data']['title'];
                                                                                                              } ?>">

                <textarea name="body" class="form-control" placeholder="body of post"><?php if (isset($data['data']['body'])) {
                    echo $data['data']['body'];
                                                                                      } ?></textarea>
